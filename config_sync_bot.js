/**
 * RAPIDHELP CONFIG SYNC BOT (V4.4 - COMPREHENSIVE JUNK CITY FILTER)
...
function sync() {
    ...
    try {
        ...
        // 1. Sync States (Locations) with Comprehensive Junk Filter
        if (hubData.locations && Array.isArray(hubData.locations)) {
            ...
            newConfig.states = hubData.locations.map(l => {
                ...
                const cleanCities = rawCities.filter(c => {
                    ...
                    // Must be longer than 1 character, not purely numeric, not in JUNK_CITIES, and not state name
        ...
        console.log(`\n✨ SUCCESS: config.json updated with clean Hub data.`);
